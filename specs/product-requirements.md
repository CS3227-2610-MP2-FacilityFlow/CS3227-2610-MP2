...
Facilities issues are often reported through conversations or scattered
messages. Requesters cannot reliably see progress, technicians lack a consistent
queue, and managers cannot prove who changed a request or how long resolution
took. FacilityFlow provides a single local desktop workflow with explicit roles,
validated state changes, and an audit trail.
...
An employee, resident, or authorized organization member who reports a facility
problem and tracks only requests they created.
...
decisions, reviews completed work, manages accounts, and views reports.

Individual team-member ownership of these roles remains TBD.

## MVP scope

- Account bootstrap, login, logout, and role-based sessions
- Manager account administration
- Request creation and requester self-service tracking
...
- Manager summary metrics and CSV report export
...
The MVP is product-complete only when a clean installation can create the first
manager, create one account for each other role, execute every valid lifecycle
transition, block representative unauthorized operations, restart without data
loss, export a report, and produce diagnostic/audit evidence without a crash.
